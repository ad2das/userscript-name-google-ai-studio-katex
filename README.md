...
- 1.13.25 cuts typing-time work: run/stop probes pre-filter buttons by label text
  ...
  collection and repair slices the moment the prompt editor becomes active again.
...
delimiter-table layout, and horizontal frame junctions share the content axes.
Original code nodes and copy text remain unchanged. Four conversation-derived
fixtures check painted connector coordinates, input/source preservation, and
horizontal reachability on desktop and mobile. Installed-browser application
still requires verification; fixture results alone do not prove live installation.
...
seconds, never exceeds ten seconds, and exits immediately during prompt input.
The bridge no longer disappears merely because one frame has no emphasis.

1.13.9 previews short pending quoted-bold spans while their closing markers
are still arriving. Only the separate live visual layer changes; the native
source, code literals, selection, and editor content stay intact. Ambiguous
unquoted tails retain the previous conservative fallback. A completed quoted
span can also paint at the streaming tail without waiting for following prose.

1.13.8 includes two neighboring containers on either side of each sampled
paragraph, so short lines between sample points also receive priority. This
uses the existing container list without extra hit tests and retains the same
per-pass processing budget. The regression explicitly excludes one paragraph
from direct hit testing and verifies it is still repaired before offscreen work.

1.13.7 captures nested conversation scrolling and prioritizes visible prose
within the existing sliced repair budget. It samples at most 21 points per
scroll-triggered refresh, without traversing all paragraph geometry. Typing and
generation guards remain before sampling or completion repairs, and the native
stabilization wait is retained. A busy-idle/450-paragraph regression checks tail
priority, bounded hit tests, and typing/streaming source preservation.
