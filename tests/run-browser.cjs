...
const run = async () => {
  ...
  try {
    ...
    const mode = (process.argv.includes('--mobile') ? 'mobile' : 'desktop') +
      (process.argv.includes('--live-controller') ? '-live-controller' : process.argv.includes('--live-preview') ? '-live-preview' : '');
