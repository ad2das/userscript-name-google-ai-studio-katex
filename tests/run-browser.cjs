...
const run = async () => {
  ...
  try {
    ...
    if (process.argv.includes('--scroll')) suites.splice(0, suites.length, 'scroll-priority-browser.test.js');
    ...
    const mode = (process.argv.includes('--mobile') ? 'mobile' : 'desktop') +
      (process.argv.includes('--t-account') ? '-t-account' : process.argv.includes('--live-typing') ? '-live-typing' : process.argv.includes('--live-integration') ? '-live-integration' : process.argv.includes('--live-controller') ? '-live-controller' : process.argv.includes('--live-preview') ? '-live-preview' : '');
